...
RPE Tracker is a velocity-based training (VBT) analysis tool that processes barbell lift videos to estimate RPE (Rate of Perceived Exertion), Mean Concentric Velocity (MCV), and projected 1RM. It uses computer vision (OpenCV CSRT tracking + Hough circle detection) with biomechanics lookup tables based on Helms et al. 2017.
...
```
POST /detect-plate  →  Hough circle detection on first video frame
...
| [backend/tracker.py](backend/tracker.py) | CSRT tracking, Hough circle detection, debug video rendering |
...
- `detectedCircle` — `{cx_norm, cy_norm, r_norm}` from Hough detection response
